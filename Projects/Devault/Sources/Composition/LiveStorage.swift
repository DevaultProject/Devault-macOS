...
enum LiveStorage {
  ...
  // shared는 static let이라 앱 실행 중 isICloudSyncEnabled가 바뀌어도 이미 만들어진 ModelContainer에는 반영되지 않는다. Settings 화면에서 토글을 지원하려면
  // TODO: - Settings 화면에서 토글을 지원하려면 재시작 안내를 띄우거나, ModelContainer를 런타임에 재생성하는 hot-swap이 필요
